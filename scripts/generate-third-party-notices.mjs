...
function goRuntimeComponents() {
  ...
  return uniqueBy(
    output
      ...
      .filter((component) => component.name !== 'foldersguard'),
...
function noticeBlocks(components) {
  ...
  for (const component of components) {
    ...
    for (const file of files) {
      blocks.push([
        `### ${component.name} ${component.version} - ${file.name}`,
        '',
        '```text',
        file.text,
        '```',
...
const notableBuildDependencies = [
  ...frontendNotableBuildDependencies().map((component) => ({
    ...component,
