...
加密内容会保留为一个可见的文件夹树，只是名称替换为UUID。 你可以把它复制到另一块硬盘、上传到云盘、只下载其中一部分，或者把选中的加密文件和目录发给别人。
...
- 空间预检：加密和解密会先计算目标磁盘所需空间，不足时拒绝开始。
- 四种源数据处理方式：保留、整个操作成功后统一删除、每完成一个文件就删除、每完成一个分片就删除。后两种会在保存设置时明确警告其会极大降低容错能力。
...
加密内容树仍然是普通文件夹树，可以通过常规存储工具移动、上传、下载或分享。真实名称会被 UUID 名称替代。 把 UUID
名称映射回真实名称所需的元数据，则单独存放在 FoldersGuard 数据中。
...
FoldersGuard 使用 SQLCipher 保护项目数据库和分享数据库。 SQLCipher 是 CGO 依赖，因此真正的发布构建必须启用 CGO，并准备好目标平台可用的
C 编译器。
