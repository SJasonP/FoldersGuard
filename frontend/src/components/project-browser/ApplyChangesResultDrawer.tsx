...
export function ApplyChangesResultDrawer({open, result, onClose, t}: ApplyChangesResultDrawerProps) {
    ...
    return (
        <Drawer title={t('applyChangesResult')} open={open} onClose={closeOrConfirm} width={760}
                maskClosable={!manualOperationGuide}>
            {result ? (
                <>
                    ...
                    <Descriptions column={1} bordered size="small">
                        ...
                        {result.operationGuidePath ? <Descriptions.Item
                            label={t('operationGuidePath')}>{result.operationGuidePath}</Descriptions.Item> : null}
                        {result.stagedContentPath ? <Descriptions.Item
                            label={t('stagedContentPath')}>{result.stagedContentPath}</Descriptions.Item> : null}
                        {manualOperationGuide ? (
                            <Descriptions.Item
